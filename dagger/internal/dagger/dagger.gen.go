...
import (
	...
	"github.com/jkjell/swf/dagger/dagger/internal/querybuilder"
)
...
// The `WitnessID` scalar type represents an identifier for an object of type Witness.
...
// Load a Witness from its ID.
func (r *Client) LoadWitnessFromID(id WitnessID) *Witness {
	q := r.query.Select("loadWitnessFromID")
	q = q.Arg("id", id)
...
func (r *Client) Witness() *Witness {
	q := r.query.Select("witness")
...
func (r *Witness) WithGraphQLQuery(q *querybuilder.Selection) *Witness {
	return &Witness{
		query: q,
	}
}

// WitnessBaseOpts contains options for Witness.Base
type WitnessBaseOpts struct {
	WitnessImg string
}

// Create base witness container
func (r *Witness) Base(opts ...WitnessBaseOpts) *Container {
	q := r.query.Select("base")
	for i := len(opts) - 1; i >= 0; i-- {
		// `witnessImg` optional argument
		if !querybuilder.IsZeroValue(opts[i].WitnessImg) {
			q = q.Arg("witnessImg", opts[i].WitnessImg)
		}
	}
...
// A unique identifier for this Witness.
func (r *Witness) ID(ctx context.Context) (WitnessID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response WitnessID
...
// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *Witness) XXX_GraphQLType() string {
	return "Witness"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *Witness) XXX_GraphQLIDType() string {
	return "WitnessID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *Witness) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
...
func (r *Witness) MarshalJSON() ([]byte, error) {
	id, err := r.ID(context.Background())
	if err != nil {
		return nil, err
	}
...
func (r *Witness) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = *dag.LoadWitnessFromID(WitnessID(id))
	return nil
}

// WitnessRunOpts contains options for Witness.Run
type WitnessRunOpts struct {
	WitnessImg string
}

// Execute and Witness a command
func (r *Witness) Run(cmdArgs []string, opts ...WitnessRunOpts) *File {
...
// WitnessVerifyOpts contains options for Witness.Verify
type WitnessVerifyOpts struct {
	WitnessImg string
}

// Execute and Witness a command
func (r *Witness) Verify(cmdArgs []string, opts ...WitnessVerifyOpts) *File {
